...
open class PlayerController(
    ...
    private val playerControllerAnimManager: IPlayerControllerAnimManager = PlayerControllerAnimManager(binding)
) : IPlayerController {
    ...
    private val lockAnimHelper = VisibleAlphaAnimHelper(binding.ivLockImg)

    init {
        if (DisplayUtils.isLandscape(binding.root.context)) {
            playerControllerAnimManager.hideControllerAnim(false)
            lockAnimHelper.setVisible(isVisible = false, useAnim = false)
        }
        binding.root.setOnClickListener {
            if (!lockAnimHelper.isVisible()) {
                lockAnimHelper.setVisible(true)
            }
            if (playerControllerAnimManager.isControllerVisible() && !binding.ivLockImg.isChecked()) {
    ...
    private val controllerCallbacks = mutableMapOf<String, ControllerCallback>()

    override fun registerControllerCallbacks(controllerCallback: ControllerCallback) {
        val key = controllerCallback.hashCode()
        controllerCallbacks[key.toString()] = controllerCallback
    }

    override fun unregisterControllerCallbacks(controllerCallback: ControllerCallback) {
        val key = controllerCallback.hashCode().toString()
        controllerCallbacks.remove(key)
    }
    ...
    override fun setPlayer(player: Player?) {
        ...
        binding.btnVideoReturn.setOnClickListener {
            notifyControllerCallbacks(ControllerAction.Close())
        }
    ...
    private fun notifyControllerCallbacks(action: ControllerAction) {
        controllerCallbacks.values.forEach {
            it.onAction(action)
        }
    }
}